...
function createLinkNode(document, cardName) {
...
PostParser.insertCardLinks = function (content) {
	...
	// Match card names exactly as they appear in the database, 
	// including punctuation, numbers, and special characters, not preceded by @username
	const cardRegex = new RegExp(`(?<!@[\S]{0,100})(${alternation})`, 'g');
	...
	function walk(node) {
		if (node.nodeType === 3) { // TEXT_NODE
			...
			// Match card names exactly as they appear in the database, including punctuation, numbers, and special characters
			const cardRegex = new RegExp(`(${alternation})`, 'g');
				const a = createLinkNode(document, cardName);
				if (a) {
					a.textContent = cardName;
					const img = document.createElement('img');
					img.src = cardData.image;
					...
					while ((match = cardRegex.exec(originalText))) {
						...
						const cardData = cardNameToData[cardName];
						if (!cardData) continue;
						// Check if match is inside a username mention (after @ and before whitespace)
						const atIdx = originalText.lastIndexOf('@', match.index);
						if (atIdx !== -1 && atIdx < match.index) {
							// Check if all chars between @ and match are non-whitespace
							const between = originalText.slice(atIdx + 1, match.index);
							if (/^\S*$/.test(between)) {
								// Inside a username, skip this match
								continue;
							}
						}
						// Push preceding text
